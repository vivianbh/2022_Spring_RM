...
def quad_sim():
    ...
    while True:
        ...
        while t <= T:
            ...
            axis = 0.5 * vee_map( des_R.T @ rel_R - rel_R.T @ des_R )
            ...
            total_moment = np.array([roll_torque, pitch_torque, yaw_torque])
            
            # motor force allocation
            control_input = np.concatenate( [np.array([total_thrust]), total_moment.reshape(3, 1)] )
