classdef controller
   methods
       function [control, error] = geometric_tracking_ctrl(~, iter, multirotor, Xd_enu, b1d)
            ...
            A =  (kx*ex + kv*ev + multirotor.m*multirotor.g*e3 - multirotor.m*ad_ned);
            b3 = R*e3;
            f = vec_dot( A, b3);
            ...
            b1d_proj = -(1/norm(b2d)) * vec_cross(b3d, b2d); 
            Rd = [b1d_proj b2d b3d];
            ...
            kR = 10;
            kW = 1;
            eR = 1/2 * vee_map(Rd'*R - R'*Rd, f);
            eW = W - R'*Rd*Wd;
            M = -kR*eR - kW*eW + vec_cross(W, multirotor.J*W) - multirotor.J*( hat_map(W)*R'*Rd*Wd - R'*Rd*Wd_dot );
